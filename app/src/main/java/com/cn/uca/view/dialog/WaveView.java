...
public class WaveView extends View {
    ...
    public WaveView(Context context, AttributeSet attrs) {
        ...
        mAboveWavePaint.setColor(getResources().getColor(R.color.ori));

        mBelowWavePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mBelowWavePaint.setAntiAlias(true);
        mBelowWavePaint.setStyle(Paint.Style.FILL);
        mBelowWavePaint.setColor(getResources().getColor(R.color.ori));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (float x = 0; x <= getWidth(); x += 20) {
             ...
             */
            y = (float) (8 * Math.cos(2*ω * x + φ) +8);
            y2 = (float) (8 * Math.sin(2*ω * x + φ));

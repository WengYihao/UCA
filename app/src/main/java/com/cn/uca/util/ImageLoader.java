...
import com.cn.uca.io.DiskLruCache;
...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = SystemUtil.getDiskCacheDir(MyApplication.getContext(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, SystemUtil.getAppVersion(MyApplication.getContext()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    class DiskCacheThread extends Thread {
        ...
        public void run() {
            try {
                String key = SystemUtil.hashKeyForDisk(path);
